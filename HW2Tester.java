...
public class HW2Tester {
	...
	private static TestCase testCases[] = {
		...
		new TestCase("setByte", 0x00000001, 0x00000000, 0x01, 0,
		                        0xFFFFFF00, 0xFFFFFFFF, 0x00, 0,
		                        0xFFFF6FFF, 0xFFFFFFFF, 0x6F, 1,
		                        0x1312E012, 0x13121312, 0xE0, 1,
		                        0x00080000, 0x00000000, 0x08, 2,
		                        0x33212284, 0x33912284, 0x21, 2,
		                        0xA4345678, 0x12345678, 0xA4, 3,
		                        0xDEADBEEF, 0xBEADBEEF, 0xDE, 3),
		                             
		new TestCase("getNibble", 0xD, 0xA84B2E9D, 0,
		                          0x9, 0xA84B2E9D, 1,
		                          0xE, 0xA84B2E9D, 2,
		                          0x2, 0xA84B2E9D, 3,
		                          0xB, 0xA84B2E9D, 4,
	...
	private static void test_strdtoi(TestCase t) {
		...
		for(int i = 0; i < t.tests.length; i += 2) {
			...
			pw.printf("%s(%10s): Expected: 0x%08X Result: 0x%08X Correct: %5b\n", t.name, arg1, exp, res, cor);
			pw.flush();
		}
	...
	private static void test_strbtoi(TestCase t) {
		...
		for(int i = 0; i < t.tests.length; i += 2) {
			...
			pw.printf("%s(%31s): Expected: 0x%08X Result: 0x%08X Correct: %5b\n", t.name, arg1, exp, res, cor);
			pw.flush();
		}
	...
	private static void test_strxtoi(TestCase t) {
		...
		for(int i = 0; i < t.tests.length; i += 2) {
			...
			pw.printf("%s(%8s): Expected: 0x%08X Result: 0x%08X Correct: %5b\n", t.name, arg1, exp, res, cor);
			pw.flush();
		}
	...
	private static void test_itostrb(TestCase t) {
		...
		for(int i = 0; i < t.tests.length; i += 2) {
			...
			int arg1 = (int) t.tests[i + 1];
			String res = HW2Bases.itostrb(arg1);
			boolean cor = exp.equals(res);
			if(cor) {
				++score;
			}
			pw.printf("%s(0x%08X): Expected: %31s Result: %31s Correct: %5b\n", t.name, arg1, exp, res, cor);
			pw.flush();
		}
	...
	private static void test_itostrx(TestCase t) {
		...
		for(int i = 0; i < t.tests.length; i += 2) {
			...
			pw.printf("%s(0x%08X): Expected: %8s Result: %8s Correct: %5b\n", t.name, arg1, exp, res, cor);
			pw.flush();
		}
	...
	private static void test_setByte(TestCase t) {
		...
		for(int i = 0; i < t.tests.length; i += 4) {
			int exp = (int) t.tests[i];
			int arg1 = (int) t.tests[i + 1];
			int arg2 = (int) t.tests[i + 2];
			int arg3 = (int) t.tests[i + 3];
			int res = HW2Operations.setByte(arg1, arg2, arg3);
			boolean cor = exp == res;
			if(cor) {
				++score;
			}
			pw.printf("%s(0x%08X, 0x%02X, %d): Expected: 0x%08X Result: 0x%08X Correct: %5b\n", t.name, arg1, arg2, arg3, exp, res, cor);
			pw.flush();
		}
	...
	private static void test_getNibble(TestCase t) {
		...
		for(int i = 0; i < t.tests.length; i += 3) {
			...
			pw.printf("%s(0x%08X, %d): Expected: 0x%1X Result: 0x%1X Correct: %5b\n", t.name, arg1, arg2, exp, res, cor);
			pw.flush();
		}
	...
	private static void test_pack(TestCase t) {
		...
		for(int i = 0; i < t.tests.length; i += 5) {
			...
			int arg2 = (int) t.tests[i + 2];
			int arg3 = (int) t.tests[i + 3];
			int arg4 = (int) t.tests[i + 4];
			int res = HW2Operations.pack(arg1, arg2, arg3, arg4);
			boolean cor = exp == res;
			if(cor) {
				++score;
			}
			pw.printf("%s(0x%1X, 0x%1X, 0x%1X, 0x%1X): Expected: 0x%04X Result: 0x%04X Correct: %5b\n", t.name, arg1, arg2, arg3, arg4, exp, res, cor);
			pw.flush();
		}
	...
	private static void test_abs(TestCase t) {
		...
		for(int i = 0; i < t.tests.length; i += 3) {
			...
			pw.printf("%s(0x%08X, %2d): Expected: 0x%08X Result: 0x%08X Correct: %5b\n", t.name, arg1, arg2, exp, res, cor);
			pw.flush();
		}
	...
	private static void test_xor(TestCase t) {
		...
		for(int i = 0; i < t.tests.length; i += 3) {
			...
			pw.printf("%s(0x%08X, 0x%08X): Expected: 0x%08X Result: 0x%08X Correct: %5b\n", t.name, arg1, arg2, exp, res, cor);
			pw.flush();
		}
	...
	private static void test_powerOf2(TestCase t) {
		...
		for(int i = 0; i < t.tests.length; i += 2) {
			...
			pw.printf("%s(0x%08X): Expected: %5b Result: %5b Correct: %5b\n", t.name, arg1, exp, res, cor);
			pw.flush();
		}
	...
	private static void test_set(TestCase t) {
		...
		int score = 0;
		for(int i = 0; i < t.tests.length; i += 3) {
			int bits = (int) t.tests[i];
			int exp = (int) t.tests[i + 1];
			int arg1 = (int) t.tests[i + 2];
			try {
			...
			pw.printf("%s(%2d): Bits: 0x%08X Expected: 0x%08X Result: 0x%08X Correct: %5b\n", t.name, arg1, bits, exp, res, cor);
			pw.flush();
		}
	...
	private static void test_isSet(TestCase t) {
		...
		for(int i = 0; i < t.tests.length; i += 3) {
			...
			pw.printf("%s(%2d): Bits: 0x%08X Expected: %5b Result: %5b Correct: %5b\n", t.name, arg1, bits, exp, res, cor);
			pw.flush();
		}
	...
	private static void test_toString(TestCase t) {
		...
		for(int i = 0; i < t.tests.length; i += 2) {
			...
			pw.printf("%s(): Bits: 0x%08X Expected: %32s Result: %32s Correct: %5b\n", t.name, bits, exp, res, cor);
			pw.flush();
		}
	...
	private static void test_onesCount(TestCase t) {
		...
		for(int i = 0; i < t.tests.length; i += 2) {
			...
			pw.printf("%s(): Bits: 0x%08X Expected: %2d Result: %2d Correct: %5b\n", t.name, bits, exp, res, cor);
			pw.flush();
		}
